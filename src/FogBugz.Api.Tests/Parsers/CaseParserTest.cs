...
using FogBugz.Api.Parsers;
using NUnit.Framework;

namespace FogBugz.Api.Tests.Parsers
{
    [TestFixture]
...
<cases count=""1"">
  ...
  <ixBug>123</ixBug> 
    ...
    <sOriginalTitle>Problem finding the goose...</sTitle> 
    ...
    <fSubscribed>true</fSubscribed> 
            ...
            Assert.AreEqual((CaseId)789, result.Children.Last());
            result.Duplicates.ShouldBeEquivalentTo((CaseId)321);
            result.Related.ShouldBeEquivalentTo(new[] { (CaseId)345, (CaseId)247, (CaseId)2920 });
            Assert.AreEqual((CaseId)654, result.Original);

            result.Tags.ShouldBeEquivalentTo("first", "second", "third");
            ...
            Assert.AreEqual((PersonId)0, result.LastEditedBy);
            ...
            Assert.AreEqual(new DateTime(2007, 05, 06, 22, 47, 59, DateTimeKind.Utc), result.FixFor.Date);
            ...
            Assert.AreEqual((MailboxId)0, result.Mailbox);
            ...
            Assert.AreEqual(0, result.DiscussionTopic);
